...
export const getAllOwnCars = async (req, res, next) => {
	try {
		const { firstname } = req.body.person;
		const user = await User.findOne({ "person.firstname": firstname });

		res.json(await Car.find({ fitUser: firstname }));
	} catch (error) {
...
export const createCar = async (req, res, next) => {
	try {
		...
		res.json({ message: "Nice to have a new car", newCar });
	} catch (error) {
